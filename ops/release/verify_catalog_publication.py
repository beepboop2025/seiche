...
CORPUS_RECEIPT_TAG_RE = re.compile(r"market-corpus-receipt-corpus-[0-9a-f]{16}")
...
def _market_corpus_publication_receipt(entry: Any) -> dict[str, Any]:
    ...
    if not (
        ...
        and receipt.get("tag") == f"{MARKET_CORPUS_RECEIPT_TAG_PREFIX}{release_id}"
...
def verify_market_corpus_receipts(
    ...
    *,
    ...
    if not (
        ...
        and _exact_int(deep.get("datasets"), signed["engineVerifiedObjects"])
    ...
    if not (
        ...
        and engine.get("verified_objects") == deep.get("datasets")
