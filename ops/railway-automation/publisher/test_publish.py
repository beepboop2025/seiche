...
class PublisherBoundaryTests(unittest.TestCase):
    ...
    def test_detached_builder_cannot_mutate_candidate_after_quiescence(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            code = "import os,time,pathlib; os.setsid(); pathlib.Path(%r).write_text('ready'); time.sleep(5); pathlib.Path(%r).symlink_to('/etc/passwd')" % (str(ready), str(late))
            child = subprocess.Popen(["setpriv", "--reuid=10001", "--regid=10001", "--init-groups", "--no-new-privs", sys.executable, "-c", code])
            try:
                for _ in range(100):
                    if ready.exists():
                        break
