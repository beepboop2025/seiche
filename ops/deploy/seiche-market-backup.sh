...
# service intentionally has no CAP_CHOWN, so an archive-preserving copy would
# fail when the live tree contains files owned by more than one service user.
# The restore path already extracts with --no-same-owner/--no-same-permissions.
