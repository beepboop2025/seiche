...
if ! timeout -k 30 600 backend/.venv/bin/pip install -q -e \
        "./backend[deploy-test,notary,collectors,postgres]" >>"$LOG" 2>&1; then
...
# Eleven files, collects 290 tests as of this commit. If a commit grows or
...
tests/test_attest.py tests/test_api_caching.py tests/test_api_v2_markets.py \
