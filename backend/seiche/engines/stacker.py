...
def event_labels(spread_bp: pd.Series, idx: pd.DatetimeIndex) -> pd.Series:
    """Same label the PROOF lab and ML Lab use: spike vs trailing 5d median
    within the next BACKTEST_EVENT_FWD_D bd. NaN where the window is open."""
    sp = spread_bp.dropna().reindex(idx)
    ...
    y[fwd_max.isna() | base.isna()] = np.nan
    return y
