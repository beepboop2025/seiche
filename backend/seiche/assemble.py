...
def seed_prebuilt_deep_cache(payload: dict) -> str | None:
    ...
    seeded["_all_ok"] = all(
        ...
        if key not in ("ok", "historical_evidence") and not str(key).startswith("_")
    )
...
def _snapshot_contains_restricted_cfets(payload: object) -> bool:
    ...
    def restricted_engine_shape(value: dict) -> bool:
        if str(value.get("harbor", "")).upper() == "CHINA" and (
            ...
            return True
        if str(value.get("basin", "")).upper() == "CHINA" and {
            "anchor",
            "value_bp",
            "z",
            "asof",
    ...
    def observed_series_has_data(value: object, *, field: str) -> bool:
        if field in RESTRICTED_SNAPSHOT_QUALITATIVE_NUMERIC_FIELDS and not isinstance(
    ...
    def quantitative_field(field: str, value: object) -> bool:
        if value is None:
            return False
        if field in RESTRICTED_SNAPSHOT_OBSERVED_SERIES_FIELDS or field.endswith(
            RESTRICTED_SNAPSHOT_OBSERVED_SERIES_SUFFIXES
        ):
    ...
    def quantitative_subtree(field: str, value: object) -> bool:
        ...
        if field in RESTRICTED_SNAPSHOT_QUALITATIVE_NUMERIC_FIELDS and isinstance(
            value, (dict, list, tuple)
        ):
            return observed_series_has_data(value, field=field)
        ...
        if isinstance(value, dict):
            ...
            for key, nested in value.items():
                ...
                if (
                    ...
                    return True
                child_prose = prose_context or field in RESTRICTED_SNAPSHOT_PROSE_FIELDS
                child_identity = (
...
async def snapshot(force: bool = False) -> dict:
    ...
    if _stateful_preactivation_read_only():
        ...
        if cached is None:
            raise RuntimeError("stateful pre-activation snapshot is not hydrated")
        return cached
