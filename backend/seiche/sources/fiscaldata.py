...
from seiche.config import FISCAL_TTL_MIN, USER_AGENT
...
async def fetch_tga_daily(client: httpx.AsyncClient, start: str = "2021-01-01") -> dict:
    ...
    if cached is None:
        try:
            rows = await _get_all_pages(
                ...
                {
                    "filter": f"record_date:gte:{start},account_type:in:(Treasury General Account (TGA) Opening Balance,Treasury General Account (TGA))",
    ...
    tga = df.dropna().drop_duplicates("record_date").set_index("record_date")["open_today_bal"].sort_index()
    return {"fetched_at": cached["fetched_at"], "tga": tga}


async def fetch_auctions(client: httpx.AsyncClient, start: str = "2023-01-01") -> dict:
