...
def test_stale_snapshot_triggers_one_refresh_then_proves_full_readiness(
    ...
    assert calls[0].startswith("readiness proof ")
    assert calls[1].startswith("readiness full ")
    assert calls[2] == "systemctl is-active --quiet seiche-api.service"
    assert calls[3].startswith("curl --fail --silent --show-error --proto =http ")
    assert calls[3].endswith("http://127.0.0.1:8787/api/gauge")
    assert calls[4] == "systemctl is-active --quiet seiche-api.service"
    assert calls[5] == calls[1]
    assert sum(call.startswith("curl ") for call in calls) == 1
    assert calls[-1] == "systemctl enable --now seiche-data-readiness.timer"
    if script_path == DEPLOY_WRAPPER:
        assert calls[-2] == f"candidate-health {'a' * 40}"
    else:
