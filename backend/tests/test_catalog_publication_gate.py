...
def _receipts(version: str = "0.13.6"):
...
def _verify(pypi, health, discovery, bodies):
    ...
    return gate.verify_public_receipts(
        "0.13.6", fetch_json=fetch_json, fetch_bytes=fetch_bytes
    )
...
def test_signed_publication_receipt_has_exact_release_generation():
    ...
    assert receipt == {
        "schemaVersion": "1.0.0",
        "tag": "market-corpus-receipt-corpus-7cb1695c6affa707-r17",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tag", "market-corpus-receipt-corpus-7cb1695c6affa707-r13"),
        ("bisBulkFlat", 1),
        ("bisApiOnly", 0),
        ("engineAttempts", 999),
...
def test_publication_receipt_tag_must_target_exact_workflow_head(monkeypatch):
    ...
    assert tag == "market-corpus-receipt-corpus-7cb1695c6affa707-r17"
    ...
    monkeypatch.setattr(
        gate, "verify_signed_release", lambda *_args, **_kwargs: "v0.13.6"
    )
    monkeypatch.setattr(
        gate,
        "_run_git",
...
def test_independent_corpus_receipt_retains_complete_application_history_checks(
    ...
    monkeypatch.setattr(gate, "verify_signed_release", lambda *_a, **_k: "v0.13.6")
    queries = []
...
def _sign_content_release(root, *, receipt=True):
    ...
    tags = ["v0.13.6", "market-corpus-v1.0.0"]
    if receipt:
...
def test_fresh_exact_head_corpus_receipt_keeps_existing_independent_release_path(
    ...
    assert (
        gate.verify_signed_release(
            root, version="0.13.6", expected_sha=head, signer_fingerprint=fingerprint
        )
        == "v0.13.6"
    )
    assert (
...
def test_signed_controller_then_daily_weekly_keeps_original_release_receipts(
    ...
    for head in (
        ...
        assert _content_git(root, "rev-parse", f"{receipt}^{{commit}}") == release
        assert _content_git(root, "rev-parse", "v0.13.6^{commit}") == release
    ...
    assert report["releaseTag"] == "v0.13.6"
...
def test_public_receipts_require_both_exact_pypi_bodies_and_live_runtime():
    ...
    assert receipt["version"] == "0.13.6"
    assert [item["filename"] for item in receipt["artifacts"]] == [
        "seiche-0.13.6-py3-none-any.whl",
        "seiche-0.13.6.tar.gz",
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "deploy/railway-ci/recovery-controller/unreviewed.py",
        "deploy/railway-ci/recovery-bootstrap/unreviewed.py",
        "deploy/railway-ci/recovery-bootstrap/nested/verify.py",
        "deploy/railway-ci/recovery-bootstrap/requirements.lock",
        "deploy/railway-ci/recovery-controller/recurring_extra.py",
        "deploy/railway-ci/recovery-controller/attest_extra.py",
        "deploy/railway-ci/recovery-controller/test_attest_extra.py",
        ...
        "backend/tests/test_railway_stateful_recovery_extra.py",
        "backend/tests/test_tide_session_alignment_extra.py",
        "backend/seiche/railway_stateful_recovery.py",
...
def test_frontend_receipt_accepts_only_reviewed_native_controller_paths(
    frontend_repo, relative
):
    root, release, _ = frontend_repo
    _frontend_change(root)
...
def test_frontend_receipt_requires_one_time_handoff_to_be_removed(frontend_repo):
    ...
    _content_git(
        ...
        "-q",
        "-m",
        "Remove completed one-time handoff",
    )
    source = _content_git(root, "rev-parse", "HEAD")
    changes = front.compatibility_changes(root, release, source)
    assert (
        sum(
            change["path"] == relative and change["kind"] == "retired_handoff"
            for change in changes
        )
        == 2
    )
...
def test_frontend_runtime_subject_remains_the_actual_backend_receipt(mutation):
    ...
    assert observed.fetch_json(
        "https://api.seiche.info/api/health?release=0.13.6",
...
def test_frontend_independent_subjects_preserve_all_original_semantic_gates(
    ...
    responses = {
        "https://api.seiche.info/api/health?release=0.13.6": (health, seiche_headers),
        "https://api.seiche.info/.well-known/mcp.json?release=0.13.6": (
            discovery,
            seiche_headers,
        ),
    ...
    observed = front.RuntimeReceipts(
        "b" * 40, version="0.13.6", corpus_release_id=release_id, request=request
    )
...
def _equivalence_tag(fixture, *, change=None, key=None, canonical=True, include_signed_frontend=False):
    payload, _, _ = _equivalence_prepare(fixture, include_signed_frontend=include_signed_frontend)
    if change:
...
def _frontend_equivalence_fixture(fixture):
    controller = _frontend_change(fixture["root"], {
...
def test_frontend_equivalence_requires_explicit_new_authority(equivalence_repo):
    ...
    assert not any(row["path"] == "frontend/src/App.tsx" for row in proof["equivalentInputManifest"]["entries"])
    assert proof["deskOverlay"]["entries"] == []


def test_frontend_equivalence_cannot_relabel_a_controller_only_receipt(equivalence_repo):
    with pytest.raises(front.Error, match="requires original-verifier-admitted frontend changes"):
...
def test_frontend_equivalence_cannot_downgrade_the_signed_purpose(equivalence_repo):
    f = _frontend_equivalence_fixture(equivalence_repo)
    tag = _equivalence_tag(f, include_signed_frontend=True, change={
        "schema": front.EQUIVALENCE_SCHEMA, "purpose": front.EQUIVALENCE_PURPOSE,
    })
    with pytest.raises(front.Error, match="non-isolated frontend"):
        _equivalence_verify(f, tag)


def test_frontend_equivalence_still_requires_original_frontend_signature(equivalence_repo):
    f = _frontend_equivalence_fixture(equivalence_repo)
    tag = _equivalence_tag(f, include_signed_frontend=True)
...
@pytest.mark.parametrize("path", ["backend/seiche/assemble.py", "frontend/package.json", "frontend/public/data/overview.json"])
def test_frontend_equivalence_never_authorizes_runtime_dependency_or_data_drift(equivalence_repo, path):
    f = _frontend_equivalence_fixture(equivalence_repo)
...
def test_frontend_equivalence_later_ui_changes_require_a_new_receipt(equivalence_repo):
    f = _frontend_equivalence_fixture(equivalence_repo)
    tag = _equivalence_tag(f, include_signed_frontend=True)
    _frontend_change(f["root"], {"frontend/src/App.tsx": "export default function App() { return 'Unreviewed change' }\n"})
    with pytest.raises(front.Error, match="generated-content"):
        _equivalence_verify(f, tag)
