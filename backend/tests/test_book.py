...
def test_stacker_label_matches_proof_event_definition(rng):
    ...
    # independent reconstruction of the PROOF/ML definition
    base = spread.rolling(5, min_periods=3).median()
    fwd = pd.concat([spread.shift(-k) for k in range(1, BACKTEST_EVENT_FWD_D + 1)], axis=1).max(axis=1)
    expect = ((fwd - base) >= BACKTEST_SPIKE_BP).astype(float)
    expect[fwd.isna() | base.isna()] = np.nan
