...
class PublicCatalogContracts(unittest.TestCase):
    ...
    def test_dcat_identity_has_a_deployable_landing_and_catalog(self) -> None:
        ...
        self.assertEqual(
            ...
            f"{REPOSITORY}/tree/v0.13.1/distribution/datasets",
        )
    ...
    def test_directory_inventory_count_and_release_order_are_explicit(self) -> None:
        ...
        for workflow, release in (
            ("publish-pypi.yml", "v0.13.1"),
            ("publish-mcp.yml", "v0.13.1"),
            ("publish-openbb.yml", "v0.12.1"),
        ):
            command = publishing.split(f"gh workflow run {workflow}", maxsplit=1)[1]
            command = command.split("```", maxsplit=1)[0]
            self.assertIn(f"--ref {release}", command)
            self.assertIn(f"release_tag={release}", command)
...
class WorkflowContracts(unittest.TestCase):
    ...
    def test_container_publication_preserves_scanned_bytes_and_attests_subjects(
        ...
        for required in (
            ...
            'docker pull --platform linux/amd64 "$IMAGE_NAME@$AMD_DIGEST"',
            'docker pull --platform linux/arm64 "$IMAGE_NAME@$ARM_DIGEST"',
            "--bundle-from-oci",
        ...
        self.assertNotIn("${{ github.token }}", prove_job)
