...
| **Bathymetry** ★ | *(v2.6 "Bathysphere")* The shape of the basin floor, measured from the water's motion — the physics program end to end on THE shared pop statistic: Kramers–Moyal drift/diffusion → the effective potential (well position, wall stiffness, escape barrier in k_BT units); the binned transition operator read as the Fokker–Planck↔Schrödinger dual (stationary density = ground state, eigenvalue moduli = energy levels, spectral gap = slowest relaxation time — critical slowing down, operator-theoretic, read on visited bins only); Schnakenberg entropy production (how hard the basin is DRIVEN from equilibrium, nats/day — the arrow of time as a gauge); and the absorbing-boundary first-passage forecast: exact P(event within h bd \| today's state) and expected days to the next event, walk-forward validated vs climatology, joining the Stack as its own member with its own record. Never composite evidence — its state variable IS the event's family. |
| **Merian Modes** ★ | *(v2.6)* The seiche eigenmodes, estimated instead of assumed. Merian's formula gives a real basin's standing-wave period from its geometry; we go the other way — Hankel-DMD (a finite-dimensional estimate of the Koopman operator: classical dynamics in the Hilbert-space clothes of Koopman–von Neumann mechanics) reads the funding basin's actual modes out of the plumbing panel: period, growth rate, current excitation. A mode with \|λ\| > 1 is a growing oscillation — instability visible before levels move; the ~21bd mode is the month-end forcing seen a second, independent way. The linear mode-propagation forecast is scored vs persistence and self-demotes (modes are structure, not a crystal ball). |
