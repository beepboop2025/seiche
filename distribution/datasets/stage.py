...
def _validate_csv(name: str, spec: dict[str, Any]) -> tuple[int, set[str], set[tuple[str, str, str]]]:
    path = spec["path"]
    _require(path.is_file() and not path.is_symlink(), f"canonical source is not a regular file: {path}")
    _require(path.parent.resolve() == OBSERVATION_ROOT.resolve(), f"source escaped reviewed directory: {path}")
    ...
    with path.open(encoding="utf-8", newline="") as handle:
        ...
        _require(tuple(reader.fieldnames or ()) == EXPECTED_FIELDS, f"field contract changed for {name}")
        for line_number, row in enumerate(reader, start=2):
            row_count += 1
            _require(row["entity"] == EXPECTED_ENTITY, f"unexpected entity in {name}:{line_number}")
            _require(row["variable"] in spec["variables"], f"unreviewed variable in {name}:{line_number}")
            try:
                date.fromisoformat(row["date"])
                value = float(row["value"])
            except (TypeError, ValueError) as exc:
                raise ValidationError(f"invalid date/value in {name}:{line_number}") from exc
            _require(math.isfinite(value), f"non-finite value in {name}:{line_number}")
            _require(row["unit"] in {"Percent", "USDollar"}, f"unexpected unit in {name}:{line_number}")
            _require(row["measurementMethod"] == spec["method"], f"method drift in {name}:{line_number}")
            _require(row["observationPeriod"] == "P1D", f"period drift in {name}:{line_number}")
            key = (row["entity"], row["variable"], row["date"])
            _require(key not in keys, f"duplicate observation key in {name}:{line_number}")
            keys.add(key)
...
def _validate_upstream_receipts() -> None:
    ...
    _require(upstream.get("schema") == "seiche.datacommons-build-manifest.v1", "unexpected upstream manifest schema")
    policy = str(upstream.get("source_policy", ""))
    _require("Direct OFR API only" in policy, "upstream source policy is no longer direct-OFR only")
    ...
    for name, spec in SOURCE_SPECS.items():
        _require(outputs[name].get("row_count") == spec["rows"], f"upstream row receipt changed for {name}")
        _require(outputs[name].get("sha256") == spec["sha256"], f"upstream hash receipt changed for {name}")
    series = upstream.get("series")
    _require(isinstance(series, list) and len(series) == EXPECTED_TOTAL_SERIES, "upstream series count changed")
    _require({item.get("variable") for item in series} == EXPECTED_VARIABLES, "upstream variable allowlist changed")
    for item in series:
        url = str(item.get("source_url", ""))
        _require(url.startswith("https://data.financialresearch.gov/v1/series/full?"), "non-OFR source URL admitted")
    ...
    candidates = [row for row in eligibility if row.get("status") == "candidate_eligible_for_review"]
    ...
    _require(candidate_mnemonics == EXPECTED_MNEMONICS, "eligible mnemonic allowlist changed")
    excluded_statuses = {row.get("status") for row in eligibility if row not in candidates}
    _require("blocked_fred_terms" in excluded_statuses, "FRED exclusion receipt is missing")
    _require("blocked_semantic_mismatch" in excluded_statuses, "primary-dealer mismatch receipt is missing")
...
def _validate_metadata() -> None:
    ...
    serialized = {name: json.dumps(document, sort_keys=True) for name, document in documents.items()}
    for name, text in serialized.items():
        _require(DOI_PATTERN.search(text) is None, f"real DOI unexpectedly present in {name}")

    manifest = documents["manifest.json"]
    _require(manifest.get("schema") == "seiche.research-distribution-manifest.v1", "kit manifest schema changed")
    ...
    for name, spec in SOURCE_SPECS.items():
        ...
        _require(row.get("sha256") == spec["sha256"], f"kit hash receipt changed for {name}")
        _require(row.get("record_count") == spec["rows"], f"kit row receipt changed for {name}")
        _require(row.get("bytes") == spec["bytes"], f"kit byte receipt changed for {name}")
        _require(row.get("content_url") == RAW_PREFIX + name, f"unpinned content URL for {name}")
        local = (KIT_ROOT / row.get("local_path", "")).resolve()
        _require(local == spec["path"].resolve(), f"local source reference changed for {name}")

    kaggle = documents["kaggle/dataset-metadata.json"]
    _require(kaggle.get("id") == "seiche-info/seiche-audited-direct-ofr-time-series", "Kaggle id changed")
    _require(kaggle.get("licenses") == [{"name": "other"}], "Kaggle rights marker changed")
    _require("NOT SUBMITTED" in str(kaggle.get("description", "")), "Kaggle draft marker missing")
    _require({item.get("path") for item in kaggle.get("resources", [])} == set(SOURCE_SPECS), "Kaggle resources changed")

    croissant = documents["croissant.json"]
    _require(croissant.get("conformsTo") == "http://mlcommons.org/croissant/1.0", "Croissant version changed")
    _require(croissant.get("seiche:publicationStatus") == "draft_not_submitted", "Croissant draft marker missing")
    _require(
        croissant.get("name") == "seiche-audited-direct-ofr-research-snapshot",
        "Croissant machine name changed",
    )
    _require(croissant.get("datePublished") == "2026-08-21", "Croissant publication date changed")
    _require(croissant.get("version") == DATASET_VERSION, "Croissant semantic version changed")
    _require(
    ...
    for term, expected in {
        ...
        if expected is not None:
            _require(actual == expected, f"Croissant context mapping changed for {term}")
        else:
            _require(term not in context, f"Croissant schema.org term was unexpectedly overridden: {term}")
    croissant_files = {item.get("name"): item for item in croissant.get("distribution", [])}
    _require(set(croissant_files) == set(SOURCE_SPECS), "Croissant distribution changed")
    record_sets = {item.get("@id"): item for item in croissant.get("recordSet", [])}
    _require(set(record_sets) == {"repo_observations", "mmf_observations"}, "Croissant record sets changed")
    _require("repo_csv" in json.dumps(record_sets["repo_observations"]), "Croissant repo extraction is unbound")
    _require("mmf_csv" in json.dumps(record_sets["mmf_observations"]), "Croissant MMF extraction is unbound")
    for name, spec in SOURCE_SPECS.items():
        _require(croissant_files[name].get("contentUrl") == RAW_PREFIX + name, f"Croissant URL changed for {name}")
        _require(croissant_files[name].get("sha256") == spec["sha256"], f"Croissant hash changed for {name}")
    ...
    resources = {Path(item.get("path", "")).name: item for item in package.get("resources", [])}
    _require(set(resources) == set(SOURCE_SPECS), "Frictionless resources changed")
    for name, spec in SOURCE_SPECS.items():
        _require(resources[name].get("path") == RAW_PREFIX + name, f"Frictionless URL changed for {name}")
        _require(resources[name].get("hash") == f"sha256:{spec['sha256']}", f"Frictionless hash changed for {name}")
        _require(resources[name].get("row_count") == spec["rows"], f"Frictionless rows changed for {name}")

    dcat = documents["dcat.jsonld"]
    dcat_text = serialized["dcat.jsonld"]
    _require(any(node.get("@type") == "dcat:Dataset" for node in dcat.get("@graph", [])), "DCAT Dataset missing")
    _require("draft_not_submitted" in dcat_text, "DCAT draft marker missing")
    dcat_datasets = [node for node in dcat.get("@graph", []) if node.get("@type") == "dcat:Dataset"]
    _require(len(dcat_datasets) == 1, "DCAT Dataset missing or duplicated")
    dcat_dataset = dcat_datasets[0]
    _require(dcat_dataset.get("dcat:version") == DATASET_VERSION, "DCAT semantic version changed")
    _require(
    ...
    for identifier, resolution in expected_distribution_types.items():
        ...
        _require(
            ...
            and checksum.get("@value") in {spec["sha256"] for spec in SOURCE_SPECS.values()},
            f"DCAT checksum lost its datatype for {identifier}",
        )

    crate = documents["ro-crate-metadata.json"]
    graph = crate.get("@graph", [])
    _require(crate.get("@context") == "https://w3id.org/ro/crate/1.3/context", "RO-Crate version changed")
    roots = [node for node in graph if node.get("@id") == "./" and node.get("@type") == "Dataset"]
    ...
    _require("not submitted" in serialized["ro-crate-metadata.json"].lower(), "RO-Crate draft marker missing")
    for name, spec in SOURCE_SPECS.items():
        _require(RAW_PREFIX + name in serialized["ro-crate-metadata.json"], f"RO-Crate URL missing for {name}")
        _require(spec["sha256"] in serialized["ro-crate-metadata.json"], f"RO-Crate hash missing for {name}")

    datacite = documents["datacite-draft.json"]
    _require(datacite.get("publicationStatus") == "draft_not_submitted", "DataCite draft marker missing")
    _require(datacite.get("doi") is None and datacite.get("url") is None, "DataCite identifiers must remain empty")
    _require(datacite.get("version") == DATASET_VERSION, "DataCite semantic version changed")
    _require(datacite.get("types", {}).get("resourceTypeGeneral") == "Dataset", "DataCite resource type changed")
    _require(
        datacite.get("creators")
        == [{"name": "Seiche", "nameType": "Organizational"}]
    ...
    _require(card.startswith("---\n") and "pretty_name:" in card, "Hugging Face card metadata missing")
    for marker in ("draft, not submitted", "No DOI", "num_examples: 11163"):
        _require(marker in card, f"Hugging Face marker missing: {marker}")
    for name, spec in SOURCE_SPECS.items():
        _require(name in card and spec["sha256"] in card, f"Hugging Face receipt missing for {name}")


def _validate_notebook() -> None:
    notebook = _load_json(NOTEBOOK)
    _require(notebook.get("nbformat") == 4, "notebook is not nbformat 4")
    _require(isinstance(notebook.get("cells"), list) and notebook["cells"], "notebook has no cells")
    all_source: list[str] = []
    for index, cell in enumerate(notebook["cells"]):
        source = cell.get("source", [])
        _require(isinstance(source, (str, list)), f"notebook cell {index} has invalid source")
        all_source.append(source if isinstance(source, str) else "".join(source))
        if cell.get("cell_type") == "code":
            _require(cell.get("execution_count") is None, f"notebook cell {index} has execution state")
            _require(cell.get("outputs") == [], f"notebook cell {index} has hidden output")
            try:
                compile(all_source[-1], f"{NOTEBOOK.name}:cell-{index}", "exec")
            except SyntaxError as exc:
                raise ValidationError(f"notebook cell {index} does not compile: {exc}") from exc
    text = "\n".join(all_source)
    _require("Authorization:" not in text and "Bearer " not in text, "notebook contains an authorization value")
    _require("api_key" not in text.lower() and "access_token" not in text.lower(), "notebook contains a credential field")
    _require("draft" in text.lower() and "not submitted" in text.lower(), "notebook draft boundary missing")
...
def validate_kit() -> dict[str, Any]:
    ...
    for name, spec in SOURCE_SPECS.items():
        ...
        _require(all_keys.isdisjoint(keys), f"duplicate keys cross source boundary in {name}")
        total_rows += rows
    ...
    _require(len(all_variables) == EXPECTED_TOTAL_SERIES, "combined series count changed")
...
def _symlink(target: Path, link: Path) -> None:
    _require(not link.exists() and not link.is_symlink(), f"staging link already exists: {link}")
...
def stage_by_reference(destination: Path) -> dict[str, Any]:
    ...
    _require(destination != REPO_ROOT.resolve(), "refusing to stage over the repository root")
    _require(destination != Path.home().resolve(), "refusing to stage over the home directory")
    if destination.exists():
        _require(destination.is_dir(), "staging destination exists and is not a directory")
        _require(not any(destination.iterdir()), "staging destination must be empty")
    else:
        destination.mkdir(parents=True)

    _symlink(KIT_ROOT / "huggingface" / "README.md", destination / "huggingface" / "README.md")
    _symlink(KIT_ROOT / "kaggle" / "dataset-metadata.json", destination / "kaggle" / "dataset-metadata.json")
    for name, spec in SOURCE_SPECS.items():
        _symlink(spec["path"], destination / "huggingface" / "data" / name)
        _symlink(spec["path"], destination / "kaggle" / name)
    for name in ("manifest.json", "croissant.json", "datapackage.json", "dcat.jsonld", "ro-crate-metadata.json", "datacite-draft.json"):
...
def main() -> int:
    ...
    action.add_argument("--validate-only", action="store_true", help="validate without creating files")
    action.add_argument("--stage", type=Path, metavar="DIRECTORY", help="create an empty symlink-only staging tree")
