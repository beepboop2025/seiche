...
function XYChart({
  xs, ys, height = 180, yLabel, vline, markers, color = P.slate,
}: {
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto", display: "block" }}>
      {yticks.map((t, i) => (
        <g key={`y${i}`}>
          ...
          <text x={padL - 6} y={Y(t) + 3} fill={P.faint} fontSize={10} textAnchor="end"
            fontFamily="Inter, sans-serif">{t.toFixed(Math.abs(yspan) < 5 ? 1 : 0)}</text>
        </g>
      ))}
      {xticks.map((t, i) => (
        <text key={`x${i}`} x={X(t)} y={H - 8} fill={P.faint} fontSize={10} textAnchor="middle"
      ...
      {yLabel && (
        <text x={12} y={padT + 10} fill={P.faint} fontSize={10} fontFamily="Inter, sans-serif">{yLabel}</text>
      )}
    </svg>
  );
}
  ...
  return (
    <div className="card span7">
      ...
      <XYChart
        ...
        vline={{ x: 10, color: P.stress, label: "event bin ≥10bp" }}
        markers={wellMarker}
      />
      <div className="kv">
      ...
      <Chart
        ...
        series={[
          { label: "relaxation τ (bd)", color: P.accentSoft },
          { label: "entropy σ (nats/bd)", color: P.strain, dash: [4, 3] },
        ]}
      />
      {v.ok && (
...
function MerianCard({ e }: { e: Any }) {
  ...
  return (
    <div className="card span5">
        ...
        <tbody>
          {(e.modes ?? []).map((m: Any, i: number) => (
            <tr key={i}>
              ...
              <td style={{ color: m.direction === "growing" ? P.stress : undefined }}>{m.direction}</td>
      ...
      <Chart
        rows={e.rows ?? []}
        series={[{ label: "dominant growth rate g*", color: P.strain }]}
        yLabel="ln|λ| /bd"
      />
  ...
  return (
    <div className="card span7">
      ...
      <XYChart
        ...
        yLabel="skill ρ by horizon (bd)" color={P.accent} height={140}
      />
      <table className="mini">
        ...
        <tbody>
          ...
          <tr><td>MAE vs persistence</td>{decay.map((d: Any) => (
            <td key={d.h} className="num" style={{ color: d.mae_ratio < 1 ? P.calm : undefined }}>{fmt(d.mae_ratio, 2)}</td>
          ))}</tr>
        </tbody>
      </table>
      ...
      <Chart
        ...
        series={[{ label: "local expansion multiplier |λ|", color: P.gold }]}
        refLine={{ value: 1.0, color: P.stress, label: "expanding" }}
      />
      {fc.point_bp != null && (
        <div className="dimsmall">
...
function RogueWaveCard({ e }: { e: Any }) {
  ...
  return (
    <div className="card span5">
      ...
      <table className="mini">
        ...
        <tbody>
          {(e.return_levels ?? []).map((r: Any) => (
            <tr key={r.years}>
              <td>{r.years}y</td>
              <td className="num" style={{ color: r.bp > (e.sample_max_bp ?? Infinity) ? P.strain : undefined }}>
                {fmt(r.bp, 0)}bp</td>
      ...
      <Chart
        rows={e.xi_rows ?? []}
        series={[{ label: "tail shape ξ (annual expanding refits)", color: P.stress }]}
      />
  ...
  return (
    <div className="card span5">
      ...
      <Chart
        ...
        series={[{ label: "branching ratio n (criticality at 1.0)", color: P.stress }]}
        yLabel="n"
      />
