...
This runbook covers **0.13.4 estuary**, which keeps source-cache work off the
request loop, publishes complete readiness evidence atomically, and verifies
refreshed BIS materializations against their own proof. Existing nullable
canonical publication clocks and the bounded
[NY Fed history import](NYFED-HISTORY-IMPORT.md) remain unchanged. The successor
must audit the current store and complete a fresh recovery after activation;
the completed historical import must not be replayed. Palimpsest numerical
China activation retains its independent offline acceptance and durability
requirements. Verify deployment and availability from exact-source receipts.
...
Four surfaces deliberately expose no share action. CORPUS is an unbounded,
...
credential state. WORKBENCH contains arbitrary provider, pair and history selections.
Their reasons are recorded under
...
metadata carries version `0.13.4`. The image dependencies remain hash-pinned
...
2. **Deploy the exact signed commit.** After Railway has accepted production
   authority, follow the [application update runbook](../ops/deploy/RAILWAY-APPLICATION-UPDATES.md).
   Fast-forward the reviewed SSH-signed release commit to `main`, preserve the
   current data, and require the signed successor activation and its fresh
   recovery/off-site proof. Public health must report the actual application SHA
   and Railway deployment. Keep the old Hetzner poller, deploy workflow and writers
   disabled; they cannot resume after Railway activation. Confirm the production
   MCP endpoint only after these deployment and data-readiness proofs pass:
   ```bash
   ...
   workflow SHA. The `r15` receipt does not change corpus version
   ...
   Keep the previous `r14` receipt on its original 0.13.3 source; `r15` binds
   this new application source to the same independently versioned corpus.
   ```bash
   release_sha="$(git rev-parse HEAD)"
   test "$release_sha" = "$(git rev-parse origin/main)"
   receipt_tag=market-corpus-receipt-corpus-7cb1695c6affa707-r15
   ...
   git tag -s -m "Seiche exact-SHA market corpus receipt r15" \
...
4. **Wait for the immutable PyPI receipt.** Do not create the GitHub Release
   ...
   for `0.13.4` and retain their server-reported SHA-256 digests:
   ```bash
   ...
   curl --fail --show-error --silent \
     https://pypi.org/pypi/seiche/0.13.4/json |
...
5. **Confirm the Zenodo GitHub integration is enabled before releasing.** Sign
   ...
   explicitly carries version `0.13.4`, language `eng`, `upload_type`, license,
   ...
   ```bash
   if gh release view v0.13.4 --repo beepboop2025/seiche >/dev/null 2>&1; then
     echo "v0.13.4 already released; inspect receipts instead of recreating it" >&2
     exit 1
   fi
   ```
...
7. **Receipt the three release-triggered surfaces.** Treat these as separate
   ...
   - **Zenodo:** wait for the integration to archive `v0.13.4`; open the record,
     confirm version `0.13.4`, software type, open access, creator, license, and
   ...
   - **MCP Registry:** watch `publish-mcp.yml`, then query the owner namespace and
     retain the workflow URL plus the returned `0.13.4` record. Recovery must use
     the tag as both workflow ref and explicit input:
     ```bash
     gh workflow run publish-mcp.yml --repo beepboop2025/seiche \
       --ref v0.13.4 -f release_tag=v0.13.4
     curl --fail --show-error --silent \
...
8. **Verify the independently published OpenBB extension.** Version `0.1.0`
   ...
   Do not dispatch another publication of that immutable version for `v0.13.4`.
