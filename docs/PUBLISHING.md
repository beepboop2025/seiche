...
This runbook covers **0.13.1 estuary**. This release adds nullable canonical
publication clocks and the bounded [NY Fed history import](NYFED-HISTORY-IMPORT.md).
The application must pass existing-store migration tests before import, and a new
post-import recovery must prove the actual extended dataset. Palimpsest numerical
...
metadata carries version `0.13.1`. The image dependencies remain hash-pinned
...
4. **Wait for the immutable PyPI receipt.** Do not create the GitHub Release
   ...
   for `0.13.1` and retain their server-reported SHA-256 digests:
   ```bash
   ...
   curl --fail --show-error --silent \
     https://pypi.org/pypi/seiche/0.13.1/json |
...
5. **Confirm the Zenodo GitHub integration is enabled before releasing.** Sign
   ...
   explicitly carries version `0.13.1`, language `eng`, `upload_type`, license,
   ...
   ```bash
   if gh release view v0.13.1 --repo beepboop2025/seiche >/dev/null 2>&1; then
     echo "v0.13.1 already released; inspect receipts instead of recreating it" >&2
     exit 1
   fi
   ```
...
7. **Receipt the three release-triggered surfaces.** Treat these as separate
   ...
   - **Zenodo:** wait for the integration to archive `v0.13.1`; open the record,
     confirm version `0.13.1`, software type, open access, creator, license, and
   ...
   - **MCP Registry:** watch `publish-mcp.yml`, then query the owner namespace and
     retain the workflow URL plus the returned `0.13.1` record. Recovery must use
     the tag as both workflow ref and explicit input:
     ```bash
     gh workflow run publish-mcp.yml --repo beepboop2025/seiche \
       --ref v0.13.1 -f release_tag=v0.13.1
     curl --fail --show-error --silent \
...
8. **Verify the independently published OpenBB extension.** Version `0.1.0`
   ...
   Do not dispatch another publication of that immutable version for `v0.13.1`.
